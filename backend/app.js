...
app.get('/', (req,res)=>{
  res.json({
    status:'OK',
    service:'Imtiyaz Node API v1.7.0',
    endpoints:['/api/paket','/api/dokumen','/api/kontak','/api/login','/api/logout','/api/jamaah/:id','/api/jamaah','/api/upload-bukti','/api/update-checklist','/api/skrining','/api/wa-admin']
  });
});

// Cache 5 menit untuk endpoint konten publik (paket, dokumen, kontak) yang sekarang
// diatur admin lewat WP Admin dan boleh sedikit delay saat propagasi ke app.
const contentCache = {};
async function fetchWithCache(cacheKey, wpPath, fallback) {
  const cached = contentCache[cacheKey];
  if (cached && Date.now() - cached.time < 300000) return cached.data;
  try {
    const r = await axios.get(`${WP_URL}/wp-json/imtiyaz/v1/${wpPath}`, { timeout: 4000 });
    contentCache[cacheKey] = { data: r.data, time: Date.now() };
    return r.data;
  } catch (e) {
    return fallback;
  }
...
app.get('/api/paket', async (req,res)=>{
  res.json(await fetchWithCache('paket', 'paket', PAKET_LIST));
});

// FIX: sebelumnya tidak ada proxy untuk /dokumen dan /kontak, padahal keduanya
// sekarang bisa diatur admin lewat WP Admin (lihat imtiyaz-connector.php v1.6.0).
// Tanpa endpoint ini, perubahan admin tidak akan pernah sampai ke aplikasi Android.
app.get('/api/dokumen', async (req,res)=>{
  const fallback = { ktp:'KTP', kk:'Kartu Keluarga', paspor:'Paspor', vaksin:'Vaksin Meningitis', foto:'Foto 4x6', nikah:'Buku Nikah' };
  res.json(await fetchWithCache('dokumen', 'dokumen', fallback));
});

app.get('/api/kontak', async (req,res)=>{
  const fallback = { nama_travel:'Imtiyaz Tour Jogja', alamat:'PPIU U383/2021 • Jln. Pertapan, Tegal Cerme RT08, Baturetno, Banguntapan, Bantul', kontak:'0811-277-6543 • pastiumrah.com' };
  res.json(await fetchWithCache('kontak', 'kontak', fallback));
});
...
// FIX: sebelumnya TIDAK ADA login sama sekali -- siapa pun yang tahu/menebak angka
// jamaah_id bisa kirim data checklist/skrining/bukti atas nama jamaah itu. Sekarang
// jamaah wajib login dengan username+password yang DIBUAT ADMIN (lewat WP Admin,
// bukan self-register) sebelum bisa memakai endpoint yang menulis data di bawah.
app.post('/api/login', async (req,res)=>{
  try{
    const r = await axios.post(`${WP_URL}/wp-json/imtiyaz/v1/login`, req.body);
    res.json(r.data);
  }catch(e){
    const status = e.response?.status || 401;
    res.status(status).json({ error: e.response?.data?.message || 'Username atau password salah' });
  }
});

app.post('/api/logout', async (req,res)=>{
  try{ const r = await axios.post(`${WP_URL}/wp-json/imtiyaz/v1/logout`, req.body); res.json(r.data); }
...
app.post('/api/upload-bukti', requireApiKey, upload.single('bukti'), async (req,res)=>{
  ...
  try{
    const { jamaah_id, token } = req.body;
    ...
    await axios.post(`${WP_URL}/wp-json/imtiyaz/v1/upload-bukti`, { jamaah_id, token, bukti_url: wpMedia.data.source_url }, { auth: WP_AUTH });
    res.json({ success:true, bukti_url: wpMedia.data.source_url, status:'Menunggu Verifikasi', message:'Bukti diterima' });
  }catch(e){
    const status = e.response?.status || 500;
    res.status(status).json({ error: e.response?.data?.message || 'Gagal upload bukti' });
  }
...
app.post('/api/update-checklist', requireApiKey, async (req,res)=>{
  try{ const r = await axios.post(`${WP_URL}/wp-json/imtiyaz/v1/update-checklist`, req.body, { auth: WP_AUTH }); res.json({ success:true, message:'Checklist tersimpan - Fitur 3', data:r.data }); }
    ...
    const status = e.response?.status || 500;
    res.status(status).json({ error: e.response?.data?.message || 'Gagal update checklist' });
  }
});
...
app.post('/api/skrining', requireApiKey, async (req,res)=>{
  try{ const r = await axios.post(`${WP_URL}/wp-json/imtiyaz/v1/skrining`, req.body, { auth: WP_AUTH }); res.json({ success:true, message:'Skrining terkirim - Fitur 8', id:r.data.id }); }
    ...
    const status = e.response?.status || 500;
    res.status(status).json({ error: e.response?.data?.message || 'Gagal kirim skrining' });
  }
});

app.listen(PORT, ()=>{ console.log(`Imtiyaz API v1.7.0 jalan di port ${PORT}`); });
