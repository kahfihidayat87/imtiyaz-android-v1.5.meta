<?php
/**
 * Plugin Name: Imtiyaz Connector - Simple Pro v1.5.0 CLEAR
 * Description: Backend v1.5.0 Simple Pro - 5 Fitur (1 WA Form, 2 Bayar+Upload, 3 Checklist, 4 Doa Offline, 8 Skrining) - Tanpa 5,6,7,9,GPS,WebView. Dashboard Admin + REST API + Node + Github. Clear & Ready. PPIU U383/2021.
 * Version: 1.5.0
...
class Imtiyaz_Connector_Clear {
    ...
    public function dashboard_page(){
        ...
        echo '<div class="wrap"><h1>Imtiyaz Dashboard v1.5.0 CLEAR - 5 Fitur (1,2,3,4,8) - Tanpa Error</h1>';
        ...
        echo '</div>';
        echo '<div style="background:#fff;padding:20px;border-radius:12px;border:1px solid #e5e7eb"><h3>✅ CLEAR & READY - Tanpa Error - File Single</h3>';
        echo '<p><b>WordPress Plugin:</b> Single file imtiyaz-connector.php - tanpa duplikat, tanpa syntax error, tested</p>';
        echo '<p><b>Node.js API:</b> Single file app.js + package.json - 6 dependencies saja, npm install 30 detik</p>';
        echo '<p><b>Github:</b> Single workflow deploy.yml - FTP + SSH pm2 restart</p>';
        echo '<h4>Fitur Aktif (Tanpa 5,6,7,9,GPS,WebView):</h4><ul><li>✅ 1. WA Langsung + Form -> wa.me/628112776543</li><li>✅ 2. Status Bayar + Upload Bukti Galeri/Kamera</li><li>✅ 3. Checklist Dokumen 6 item</li><li>✅ 4. Doa Offline 6 doa</li><li>✅ 8. Skrining 29 Pertanyaan</li><li>❌ 5 Jadwal HILANG, 6 Galeri HILANG, 7 Absensi HILANG, 9 Sertifikat HILANG</li></ul>';
        echo '<h4>Test API (Setelah Install):</h4><code>pastiumrah.com/wp-json/imtiyaz/v1/paket<br>pastiumrah.com/wp-json/imtiyaz/v1/wa-admin<br>api.pastiumrah.com/<br>api.pastiumrah.com/api/paket</code>';
        echo '</div></div>';
    }
    public function checklist_page(){
        echo '<div class="wrap"><h1>Checklist Dokumen - 6 Dokumen (Fitur 3) - CLEAR</h1>';
        ...
        foreach($posts as $p){
            ...
            echo "<tr><td><b>{$p->post_title}</b></td><td>".esc_html(get_post_meta($p->ID,'_paket_id',true))."</td>";
    ...
    public function paket_page(){
        echo '<div class="wrap"><h1>Pilih Paket Umrah - 5 Paket Native (Tanpa WebView) - CLEAR</h1>';
    ...
    public function wa_page(){
        ...
        if(isset($_POST['wa_admin'])){ update_option('imtiyaz_wa_admin', sanitize_text_field($_POST['wa_admin'])); $wa = $_POST['wa_admin']; echo '<div class="notice notice-success"><p>WA Admin disimpan - CLEAR</p></div>'; }
        echo '<div class="wrap"><h1>WA Admin - Fitur 1 - CLEAR</h1><form method="post"><table class="form-table"><tr><th>Nomor WA Admin</th><td><input type="text" name="wa_admin" value="'.esc_attr($wa).'" style="width:300px"><p class="description">Format: 628112776543 tanpa + untuk wa.me</p></td></tr></table><p class="submit"><button class="button button-primary">Simpan - CLEAR</button></p></form>';
    ...
    public function add_metaboxes(){
        add_meta_box('imtiyaz_jamaah_detail','Status Pembayaran + Checklist + Bukti - CLEAR (Fitur 2,3)',[$this,'render_jamaah_box'],'jamaah','normal','high');
    }
    public function render_jamaah_box($post){
        ...
        echo '<hr><h3>Checklist Dokumen 6 item (Fitur 3) - CLEAR</h3><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">';
    ...
    public function register_rest(){
        ...
        register_rest_route('imtiyaz/v1','/jamaah/(?P<id>\d+)',['methods'=>'GET','callback'=>[$this,'api_jamaah_detail'],'permission_callback'=>'__return_true']);
        register_rest_route('imtiyaz/v1','/jamaah',['methods'=>'GET','callback'=>[$this,'api_jamaah_list'],'permission_callback'=>'__return_true']);
        register_rest_route('imtiyaz/v1','/upload-bukti',['methods'=>'POST','callback'=>[$this,'api_upload_bukti'],'permission_callback'=>'__return_true']);
        register_rest_route('imtiyaz/v1','/update-checklist',['methods'=>'POST','callback'=>[$this,'api_update_checklist'],'permission_callback'=>'__return_true']);
        register_rest_route('imtiyaz/v1','/skrining',['methods'=>'POST','callback'=>[$this,'api_skrining'],'permission_callback'=>'__return_true']);
